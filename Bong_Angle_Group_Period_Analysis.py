#!/usr/bin/env python3
"""
Analyze bond-angle RMSE deviations by periodic-table group and period.

Input:
    Overall_Metal_Bond_Angle_RMSE_summary.csv
...
By default, Mean_RMSE_deg is used as the RMSE metric.
...
INPUT_CSV = os.path.join(SCRIPT_DIR, "Overall_Metal_Bond_Angle_RMSE_summary.csv")
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Group_Period_Analysis")

RMSE_COLUMN = "Mean_RMSE_deg"
RMSE_LABEL = "Mean bond-angle RMSE (°)"
...
def main():
    ...
    data.to_csv(
        os.path.join(OUTPUT_DIR, "Metal_Bond_Angle_RMSE_with_Group_Period.csv"),
        index=False
    )
    ...
    group_summary.to_csv(
        os.path.join(OUTPUT_DIR, "Group_Wise_Bond_Angle_RMSE_Summary.csv"),
        index=False
    )
    ...
    period_summary.to_csv(
        os.path.join(OUTPUT_DIR, "Period_Wise_Bond_Angle_RMSE_Summary.csv"),
        index=False
    )
    ...
    values = group_summary["Mean_RMSE_deg"].to_numpy()
    ...
    ax.set_title(
        "Group-wise Bond-Angle RMSE: UMA vs VASP",
        fontsize=TITLE_SIZE,
        fontweight="bold",
        pad=14
    )
    ...
    save_plot(fig, "Group_Wise_Bond_Angle_RMSE")
    ...
    values = period_summary["Mean_RMSE_deg"].to_numpy()
    ...
    ax.set_ylabel(RMSE_LABEL, fontsize=AXIS_LABEL_SIZE, fontweight="bold")
    ax.set_title(
        "Period-wise Bond-Angle RMSE: UMA vs VASP",
        fontsize=TITLE_SIZE,
        fontweight="bold",
        pad=14
    )
    ...
    annotate_bars(ax, bars, values, ymax)
    apply_adsorbed_style(ax)
    save_plot(fig, "Period_Wise_Bond_Angle_RMSE")
    ...
    ax.set_ylim(0, 27)
    apply_adsorbed_style(ax)
    save_plot(fig, "Metal_Wise_Bond_Angle_RMSE_Ordered_by_Group")
    ...
    ax.set_ylim(0, 27)
    apply_adsorbed_style(ax)
    save_plot(fig, "Metal_Wise_Bond_Angle_RMSE_Ordered_by_Period")
